...
ALTER PROCEDURE sp_leer_producto
...
BEGIN
	...
	BEGIN
		SELECT	id_producto, nombre, descripcion,
				fecha_creacion, estado, imagen, precio
		FROM	producto
	END
	ELSE
	BEGIN
...
AS
BEGIN
	INSERT INTO producto(nombre, descripcion, fecha_creacion, imagen, precio)
	VALUES (@nombre, @descripcion, GETDATE(), @imagen, @precio)
...
BEGIN
	...
	BEGIN
		...
		IF @imagen is NULL
		BEGIN
			SET @imagen = (select top 1 imagen from #tmp_producto)
		END

		IF @precio is NULL
		BEGIN
			SET @precio = (select top 1 precio from #tmp_producto)
		END
...
-- EXEC sp_inserta_producto 'TOSHIBA T300', 'Laptop de gama media'
--CREATE PROCEDURE sp_inserta_producto
ALTER PROCEDURE sp_leer_categoria
